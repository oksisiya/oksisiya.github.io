...
![Screenshot](/assets/img/2026-01-21/스크린샷 2026-01-22 오후 1.09.08.png)|![Screenshot](/assets/img/2026-01-21/스크린샷 2026-01-22 오후 1.09.10.png)

<br>

첫 번째 방법은 위치 당 하나의 임베딩(one embedding per position)을 사용하고 그 임베딩을 학습하는 것이다. 두 번째 방법은 위치 당 하나의 임베딩을 사용하지만 그 임베딩을 학습하는 것은 아니다. 미리 정해진 것(something that is predetermined)을 사용하게 된다. 저자들은 사인 함수와 코사인 함수를 사용한 공식을 선택했다. 여기서 핵심은 주어진 위치($\text{m}$)에 대해 크기가 $\text{d}$인 벡터 모델(a vector of size d model)을 만드는 것이다. 토큰들을 더해야(adding them 추가해야) 하기 때문에 $\text{d}$는 토큰 임베딩의 차원과 일치해야 한다. 그리고 모든 인덱스에 대해 이 공식들에 대한 값(the corresponding value with respect to these formulas)을 계산하게 된다. 이 공식들은 뭘까? 첫 번째 공식은 어떤 것과 m의 곱에 사인(sine of something times m)을 취한 것이다. 두 번째 공식은 어떤 것과 m의 곱에 코사인(cosine of something times m)을 취한 것이다.


표기법을 단순화하자. $\omega_i=10000^{-\frac{2i}{d_{\text{model}}}}$라 가정하자. 즉 $\omega_i$가 $i$ 곱하기 $m$의 함수(omega as a function of i times m)라고 가정해보자. $\omega_i$가 $10000^{-\frac{2i}{d_{\text{model}}}}$ 양(quantity)임을 기억하라. 이런 식으로 임베딩을 구성한다고 가정하자.
...
우리가 원하는 것은 서로 가까이 있는 단어일수록 관련성이 높을 가능성이 크고 멀리 있는 단어일수록 관련성이 낮을 가능성이 크다는(words that are close together are liekly to be more relevant, as opposed to words that are further together) 사실을 반영하는 식으로 위치를 나타내는 것이다. 즉 두 단어가 한 위치만 떨어져 있는 경우와 10,000 위치 떨어져 있는 경우(one position apart versus 10,000 position apart)가 있다면 한 위치만 떨어진 단어가 더 유사해야 한다(one position apart is more similar than the other one). 이 공식이 타당한지 살펴보자. 두 개의 위치 임베딩이 있다. 하나는 위치 m에 하나는 위치 n에 있다.. 그리고 미리 정해진 공식을 사용하여 모든 값을 계산했다고 가정해보자.
...
알고 보니 이 양(quantity)($\cos(\omega_i(m-n))$)은 두 위치 임베딩의 내적을 계산할 때 나타나는 여러 요소 중 하나(one component)일 뿐이다.

왜냐하면 기본적으로 위치 m과 위치 n을 내적할 때 먼저 첫 번째 위치의 값을 곱하고, 두 번째 위치에 값을 더하고, 다시 곱하는 식으로 계속한다(take the first position, multiply them, then plus the second position, multiply them, et cetera, et cetera). 그리고 이것이 $\cos(\omega_i(m-n))$ 양(quantiy)임을 알 수 있다.

결국 우리는 이러한 임베딩들의 내적을 수행하면 m과 n 사이의 상대 거리에 대한 함수인 cosine 값의 합(a sum of cosine that are a function of the relative distance between m and n)이 나온다는 것을 알 수 있다.

<br>

Q. 두 임베딩이 가까울수록 더 유사하다는 것인가? (The closer they are, the more similar they are?)

A. 직관적으로 말하자면 이러한 임베딩 공식화 방법은(way of formulating the embeddings) 근사화하거나(approximate) 모방하려는(mimic) 것이다.
...
그래서 내적($\left\langle \text{PE}_m, \text{PE}_n \right\rangle$)을 얻게 되는데 이는 m과 n, 즉 두 임베딩 사이의 상대 거리에 대한 함수이다.

다시 한번 상기하자면 왜 내적에 관심을 가져야(why do I care) 할까? 임베딩 세계에서(in the embedding world) 두 임베딩 간의 유사성(similarity)을 정량화하려고(quantify) 할 때 일반적으로 두 임베딩의 내적이 개입된다(involving). 일반적으로 코사인 유사도(cosine similarity)를 사용한다. 코사인 유사도는 각 임베딩의 노름(norm)에 대한 내적이다. 그러니까 기본적으로 내적과 같다. 그래서 우리가 내적에 관심을 갖는 것이다. 그리고 여기서 알 수 있듯이 이것은 두 지점 사이의 상대적인 거리에 따른 함수(a function of the relative distance between the two)이다.

특히 $\cos(0)=1$이다. 이 숫자(number)($\cos(\omega_i(m-n))$)가 클수록 이 숫자의 코사인 값은 작아진다(the higher this number, the lower the value of cosine of this number). 물론 주기성이 있기(periodic) 때문에 이 말이 반드시 맞는 것은 아니다. $\pi$를 지나면 방향은 완전히 반대가 된다. 말하고자 하는 것은 m과 n이 같을 때 $\cos(0)$에 대한 합을 얻게 된다는 것이다. 그리고 그 값은 내적($\left\langle \text{PE}_m, \text{PE}_n \right\rangle$)이 최대(maximum)가 되는 값이다. 즉 m과 n이 같을 때 이 값이 최대가 된다. 즉 위치 자체만 놓고 보면(looking at the position itself) 가장 유사하다는(the most similar) 뜻이다.
...
임베딩 값(values of embeddings)을 그래프로 나타내면 다음과 같다. 왼쪽 그래프에서 y축은 50개 위치 각각에 대한 모든 임베딩을 나타낸다. x축은 주어진 벡터의 여러 차원에 걸친 값들을 나타낸다. 예를 들어 첫 번째 행을 보면 벡터를 어떻게 인덱싱 하는지에 따라(depending on how you index your vector) 첫 번째 위치 또는 0번째 위치(fisrt position or number 0 position)를 보고 있는 것이다. 그리고 위치 0 임베딩(position 0 embedding)의 모든 값을 보고 있다(are looking). 보이는 것과 같이 차원이 낮을수록 이 값은 매우 빈번하게 오르내린다 (for low dimensions, this value goes up and down very frequently). 즉 더 높은 주파수(high frequency)이다. 그리고 차원이 높을수록 값이 오르내리는 데 많은 시간이 걸린다 (when the dimension is high, it basically takes a lot of time for the value to go up and down). 즉 더 낮은 주파수(low frequency)이다.

이것은 앞서 언급했던 $\omega_i$와 관련있다. $\omega_i$는 차원 $i$의 값이 작을 때 매우 높다 very high for low values of i, which is the dimension. 그리고 $i$의 값이 클 때는 매우 낮다 very low for high values of i. 즉 오메가는 코사인과 사인 값이 얼마나 빨리 변하는지를 결정한다 (basically just determines how quickly your cosine and shine basically vary).
...
이것이 바로 원저자들(original authors)이 시도했던 것이다. 그리고 이러한 방법들을 사용하면 학습된 방법(learned one)과 비슷한(comparable) 결과가 나온다는 점에 주목했다. 하지만 이 방법은 큰 장점이 있다. 훈련 시점에 관찰했던 시퀀스 길이 뿐만 아니라 어떤 시퀀스 길이에도 적용할 수 있기 때문이다 can extend to any sequence length, not just a sequence length that you saw at training time. 이것이 바로 이 방식이 더 바람직한(preferable) 이유 중 하나이다. 이것이 저자들이 선택한 방식(what the authors chose)이다.

2025년으로 시간을 되돌려 보자(fast forward to 2025). 아직도 이 방법을 사용하고 있는지("Are we still using that?") 물을 수도 있다. 어느 정도는 그렇다. 우리는 여전히 멀리 떨어진(far) 토큰일수록 가까운(closer) 토큰보다 유사성이 낮도록 하는 아이디어를 사용하고 있다 far tokens to be less similar than closer tokens. 하지만 우리는 저자들이 한 것처럼 임베딩을 삽입하지는(injecting) 않는다. 그 이유를 알아보자.

<br>

![Screenshot](/assets/img/2026-01-21/스크린샷 2026-01-22 오후 1.10.53.png)
