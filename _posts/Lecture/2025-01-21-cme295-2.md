...
우리가 관심 갖는 것은 셀프 어텐션 계산에서 토큰들이 얼마나 유사한지(how similar tokens are)를 결정하는 것이다. 셀프 어텐션 계산은 어텐션 레이어에서 이루어진다. 우리는 임베딩을 계산해서 Input Embedding 레이어에 더한다고(add) 했다. 하지만 사실 우리가 원하는 건 어텐션(Multi-Head Attention) 레이어에 이 유사성을 반영하는(reflect) 것이다.
