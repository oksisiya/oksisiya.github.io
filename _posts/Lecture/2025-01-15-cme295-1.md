---
title: "[Stanford][CME 295] Lecture #1 Transformer"
...
그래서 RNN은 여러 가지 작업(task)에 사용되었다. 우리가 이전에 봤던 카테고리들과 다시 매핑해 보면 다음과 같다. 분류의 경우 문장 안에서 마지막 단어의 은닉 상태(hidden state of last word in your sentence)를 사용할 수 있다. 예를 들어 리뷰의 감성(sentiment of a review)을 예측한다고 해보자. 그러면 말 그대로 여기서 마지막 벡터(last vector)를 가져와서 그 벡터를 우리가 예측하고 싶은 라벨 공간으로(into the space of the predictions or the labels) 투영하면(project) 된다. 예를 들어 positive 또는 negative를 예측하고 싶다면 그 벡터를 해당 공간으로 투영한다. 다중 분류의 경우도 마찬가지로 관심 있는 토큰의 표현(representation of the token of interest)을 가지고 그것을 투영한다. 생성의 경우에는 전체 소스 텍스트(the whole source text)를 처리한 다음 처리 마지막에 얻은 context vector (a.k.a activation vector, a.k.a hidden state)를 가지고 출력 예측(output prediction)을 디코딩(decode)하게 된다. 이런 식으로 각각의 작업에서 RNN을 사용할 수 있다. 그런데 요즘에 RNNs에 대해 애야기를 거의 듣지 못하는 이유는 RNN에는 장점도 있지만 단점이 매우 많았기 때문이다. 그중 하나의 단점은 문장의 의미가(the meaning of the sentence) 전부(solely) hidden state에만 압축되어(encapsultated) 있다는 것이다. 이로 인해 장기 의존성(long-range dependencies) 문제가 발생한다. 이 문제는 모델이 과거에 무엇을 보았는지 "기억(remember)"하는 능력에 영향을 미치며, 이것이 바로 RNNs을 기반으로 구축된 또 다른 유형의 모델들이 존재하는 이유이다. 그것이 바로 LSTMs(Long Short-Term Memory)이다. 이 확장의 목표는 우리가 앞서 이야기 했던 hidden state 위에 기억해야 할 "중요한(important)" 것들을 추적할 수 있는 방법을 마련하는 것이다. 여기서 a_t는 activation으로 지금까지의 시퀀스가 인코딩된 값이다. 그리고 추적하는 또 다른 값(quantity)인 cell state가 있는데, 이 값은 여기서 c로 표기된다. 그래서 이 아키텍처는(architecture) 이러한 문제를 개선하려는 목적을 가지고 있었지만 이것도 완벽한 해결책은 아니었다.

<br>

RNN 기반 방법들의 주요 문제는(main issue of RNN-based methods) 과거의 데이터를 잊어버린다는(forgetting) 것이었다. 문헌에서는(in the literature) 이러한 현상을 기울기 소실(vanishing gradient)이라고 부른다.

그렇게 불리는 이유는 다음과 같다. 예를 들어 마지막 단어(the last words)를 예측하려면 그 이전에 나온 모든 은닉 상태(hidden state)에 의존해야 한다. 모델의 가중치(weights)를 업데이트 해서 예측값(prediction)과 실제 정답(actual prediction)을 일치시킬 때마다 역전파(backpropagation)를 수행해야 하는데 이때 여기서의 값(the value here)은 단순히 이 계산뿐만 아니라(not only a matter of this computation) 순차적으로 발생한 계산들의 결과도(but also this computation that basically happended in a sequnetial manner) 고려해야 한다. 그래서 시간을 거슬러 역전파하려는 현상(phenomenon of tryting to back-propagate through time)이 나타난다. 하지만 문제는 실제로 그 식을 적어보면 - 굉장히 지저분한 수식(a very ugly formula)이 되는데 - 결국 여러 개의 값들을 계속 곱하는 형태(a product of a bunch of quantities)가 된다. 이 때 그 값이 1보다 크면 (greater than 1) 폭주하고(exploding) 1보다 작으면 (less than 1) 소실된다(vanishing). 왜냐하면 1보다 작은 값들을 곱하면 결국 0에 가까워지기 때문이다. 그러니까 만약 업데이트 하려는 값이 0이 된다면 업데이트 자체에 문제가 생긴다. 이건 대략적인 직관(high-level intuition)이다. 이 수업의 주요 주제가 아니기 때문에 이 지저분한 수식에 대해 자세히 다루지는 않는다. 하지만 아마도 이러한 순차적인 특성(sequential characteristic) 때문에 과거의 일을 기억하는 데 있어서 이 방식이 그렇게 효과적이지 않다는 점을 이해하면 좋겠다.
