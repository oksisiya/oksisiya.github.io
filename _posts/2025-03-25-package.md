...
추가로, 모든 패키지가 아닌 특정 패키지만 빌드 할 수도 있다. 지금은 하나의 패키지만 존재하기 때문에 한번에 빌드 해도 괜찮다. 하지만 패키지가 많아졌을 때 한번에 빌드 하게 되면 수정 사항이 없더라도 모든 패키지에 대해 변동 사항이 있는지 빌드 시스템이 확인하기 때문에 빌드 하는 데 많은 시간이 소요된다.
