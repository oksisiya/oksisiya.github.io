...
* **Open X-Embodiment**
    * 로봇 조작(manipulation)을 위한 교차 구현(cross embodiment)(me: 로봇이 달라져도 하나의 모델로 제어하는 기술) 데이터셋
    * 다양한 조작 작업(various manipulation tasks), 언어 조건부 제어(language conditioned control) 및 로봇-환경 상호작용(robot-environment interactions)

* **AgiBot-Alpha**
    * 100대의 로봇으로부터 수집된 대규모 궤적(trajectories) 데이터셋
    * 정밀한 조작(fine-grained manipulation), 도구 사용(tool usage) 및 다중 로봇 협업(multi-robot collaboration)
...
인간 비디오 데이터셋(human video dataset)에는 명시적인 행동 레이블(action labels)은 포함되어 있지 않지만 인간-사물 간 상호작용(human-object interactions)을 담은 광범위한 시퀀스가 포함되어 있어 행동 양상(affordances), 작업 의미론(task semantics), 그리고 자연스러운 동작 패턴(natural motion patterns) 등을 포착한다. 파지(grasping), 도구 사용(tool use), 요리(cooking), 조립(assembly) 및 자연 환경에서 수행되는 기타 작업 지향적(task-oriented) 활동들을 포함한 광범위한 실제 환경에서의 인간 행동(real-world human behaviors)을 다루며 손-물체 간 상호 작용(hand-object interactions)에 대한 자세한 1인칭 관점(first-person perspectives)을 제공한다.
