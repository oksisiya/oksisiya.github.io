...
**Diffusion Model**
* A powerful generative model capable of capturing highly complex probabilistic distributions
* Use as the base model

<br>

**DDPM (Denoising Diffusion Probabilistic Model)**
* Defines a **forward process** that gradually **adds noise** to the data sample $x_0 \sim p_{\text{data}}(x)$:


$$
x_t = \alpha_t \, x_{t-1} + \sqrt{1 - \alpha_t} \, \epsilon_t
$$


<center>($\alpha_t$: noising schedule)</center>

$$
x_t \sim \mathcal{N}(\bar{\alpha}_t x_0, (1 - \bar{\alpha}_t) I)
$$


<center>where $\quad \bar{\alpha}_t = \prod_{s=1}^{t} \alpha_s \quad$ goes to $0$ as $\quad t \to +\infty$
</center>


* Trains a model $\mu_\theta(x_t, t)$ to **predict denoised sample** $x_0$ given the noised sample $x_t$ with its timestep $t$
* **Generates the sample** by removing the noise through a **reverse diffusion process**:
...
**Guided Sampling**
* Supported by diffusion models to **generate data samples faithfully**
* Sample from a product probability distribution $p(x)$ when $p_{\text{data}}(x)$ is modeled by a diffusion model


$$
p(x) \propto p_{\text{data}}(x) \, h(x)
$$


<center>($h(x)$: a differentiable energy function $h(x) = \exp(J(x))$)</center>

* A small modification to the reverse diffusion step by **adding a correction term** $\alpha \, \Sigma \, \nabla J(\mu)$ to $\mu$
    * $\alpha$: a step size hyperparameter, $\Sigma$: the variance in each diffusion step
    * Guide the sample **towards high-energy regions** in the sample space
* Set $h(x)$ to control the style of generated samples


<br>

**Robot System and Notations**
* Assume the robot hand is driven by a widely used PD controller
    * Command a joint target position $\tilde{q}_t$s
    * Use torque $\tau$ to drive the joints


$$\tau = K_p(\tilde{q}_t - q_t) - K_d \dot{q}_t$$


<center>($q_t$: the current joint position, $\dot{q}_t$: the joint velocity, $K_p$ and $K_d$: constant scalar gains)</center>

* Use $x_t$ to denote the key point positions of finger links at time $t$ in the wrist frame
...
**A Large-Scale Behavior Dataset**
* To pretrain the DexGen model, **ensuring comprehensive coverage of the state space**
* Accomplish this by collecting **object manipulation trajectories in simulation through reinforcement learning**

<br>

> Fig. 5: Our large-scale, multi-task pretraining dataset covers diverse grasp to grasp transitions (arrows).
>
![Figure 5](/assets/img/2025-11-19-dexgen/figure-5.png)
> * Action distribution (purple shaded area) learned by DexGen controller at each state
> * Sequential motion prompting (purple triangle) to perform a useful long-horizon skill, connecting two distance states
...
* Definition of the training task
    1. <U>Generate a set of object grasps</U> using Grasp Analysis and Rapidly-exploring Random Tree (RRT)  
        ⇨ A tuple (hand joint position, object pose)
    2. <U>Initialize the object</U> in the hand <U>with a random grasp</U> in each RL rollout
    3. <U>Set the goal</U> to be a randomly selected <U>nearby grasp</U> using the <u>k Nearest-Neighbor search<u>
    4. <U>Update the goal</U> in the same way after reaching the current grasp goal
* Crucial to **select a nearby reachable goal** during the training process, as learning to reach a distant grasp directly can be difficult
...
* Use this anygrasp-to-anygrasp policy
    * Sample over 100K grasp for most objects during grasp generation to cover **all the possible hand-object interaction modes**
    * Yields **a rich repertoire of useful skills**, including object translation and reorientation, which the **high-level policy can leverage for solving downstream tasks**
    * Also introduce other tasks such as free finger moving and fine-grained manipulation (e.g., fine rotation) to handle tasks that have special precision requirements
* Use a diverse set of random objects and wrist poses
    ...
    * Randomly adjust the **wrist to different poses** throughout the process
* Can **manipulate different kinds of objects in different wrist configurations against gravity** rather than being limited to manipulating a single object at a certain pose by combining all these data

<br>

**Simulation Dataset**
* Collect a total of 1 × 10¹⁰ transitions
* Requires 300 GPU hours to rollout trained RL policies
...
* **Gradient guidance**
    ...
    * Then convert the generated finger keypoint movement to action by the inverse dynamics model
* DDIM sampler during inference for 10Hz control
    * Total sampling time: around 27ms (37Hz)
    * On a Lambda workstation equipped with an NVIDIA RTX 4090 GPU
...
![Figure 6](/assets/img/2025-11-19-dexgen/figure-6.png)
...
* Simulate <u>2 kinds of suboptimal policies</u> with an expert RL policy $\pi_{\exp}$
    ...
    * Compare these suboptimal experts $\pi$ to their <u>assisted counterparts $\text{DexGen} \circ \pi$<u>
...
![Memo 1](/assets/img/2025-11-19-dexgen/memo-1.png)

<br>

> Fig. 8: Results of simulation evaluation
>
![Figure 8](/assets/img/2025-11-19-dexgen/figure-8.png)
>
> \* Each dimension of action space bounded by $[-1, 1]$<br>
> \* Average duration (in seconds) and number of achieved goals per trial over a 20-minute simulated experiment
...
* First set: common in-hand dexterous manipulation behavior (<u>atomic skills</u>)
...
* Second set: more <u>realistinc, long-horizon tasks</u>
...
|Functional grasping (regrasping)|Perform a power grasp on the tool handle placed either horizontally (normal)<br>or vertically in the air (horizontal functional grasp)|
|In-hand regrasping<br>(A harder version of object reorientation)|Achieve a specific grasp configuration (object pose + finger pose)|
...
**Evaluation Protocol**
* Evaluate the **performance of a teleoperation system** by measure the **success rate a human user can achieve** when using it to solve certain tasks
...
>
> \* Success rate (SR) and time-to-fall (TTF) / Holding Time metric which is normalized by the test episode length
...
> TABLE 2: The breakdown success analysis of syringe and screwdriver teleoperation
>
...
* Achievig **tool use**
    * **Challenging** as it **involves several stages** of complex dexterous manipulation
    * Achieve a reasonable stage-wise success rate, but **chaining these skills together is difficult**
* Believe that <mark><strong>improving stage-wise policy</strong> in the future can eventually <strong>close the gap</strong></mark>
...
2. Vision: Hand-Eye Coordination (leave this to future research)
    ...
    * <mark>Should be in the <strong>high-level policy</strong> or part of the proposed <strong>foundation low-level controller</strong>?</mark>
