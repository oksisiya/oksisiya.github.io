...
* Cyberbotics
* 굉장히 깔끔한 GUI. 사용자 친화적인 GUI. 코드를 보면서 시뮬레이션 할 수 있음
* 크로스 플랫폼. 윈도우나 맥 os, 리눅스를 모두 지원
* 다양한 로봇 모델이나 센서 라이브러리도 내장되어 있다 보니 로봇 공학에 대한 교육이나 연구에 대한 부분. 프로토타입핑 만드는 부분. 로봇 대회 나가는 부분에 활용
* 물리 엔진 오디를 기반으로 했는데 커스텀 엔저ㅣㄴ을 사용해서 좀 더 개선한
* 렌더링 같은 경우는 좀 더 사실적인. 가제보 보다도 사실적인 렌더링을 표현.
* api 같은 경우 굉장히 확장성이 높은데. C++, Python, 자바나 매트랩까지도.
* 마찬가지로 ros2 인터페이스 제공
* 오픈 소스임
* 사용 편의성이라든가 문서화 굉장히 잘 돼 잇음. 개발하기 편리한.
* 모델 라이브러리 풍부.
* 다만 접촉 동역학의 성능이라든가 정확성에 대해서 차이가 있을 수 있고.
